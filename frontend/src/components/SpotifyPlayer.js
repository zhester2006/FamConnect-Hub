...
export default function SpotifyPlayer() {
  ...
  return (
    <div className="glass-card rounded-2xl p-6" data-testid="spotify-player">
      <div className="flex items-center space-x-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">
            {isConnected ? 'Connected' : 'Demo Mode - Add Spotify keys to connect'}
          </p>
        </div>
      </div>
      ...
      {currentTrack ? (
        <div className="space-y-4">
          ...
          <div className="w-full aspect-square rounded-xl bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center">
            <Music className="w-16 h-16 text-white opacity-50" />
          </div>

          {/* Track Info */}
          <div className="text-center">
            <h4 className="font-bold text-white text-lg mb-1">{currentTrack.name}</h4>
            <p className="text-sm text-slate-400">{currentTrack.artist}</p>
            <p className="text-xs text-slate-500 mt-1">{currentTrack.album}</p>
          </div>
          ...
          {!isConnected && (
            <div className="mt-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
              <p className="text-xs text-yellow-400 text-center">
                🎵 Add your Spotify API keys in Settings to play real music!
              </p>
            </div>
          )}
        </div>
        ...
        <div className="text-center py-8">
          ...
          <p className="text-slate-400 text-sm">No track loaded</p>
        </div>
      )}
    </div>
  );
