  ...
  return (
    <div className="flex h-screen" style={{ backgroundColor: 'var(--color-bg)' }}>
      ...
      <main className={`flex-1 overflow-y-auto transition-all duration-300 ${sidebarCollapsed ? 'md:ml-16' : 'md:ml-64'}`}>
        <div className="p-4 pt-16 md:pt-4 lg:p-6 lg:pt-6 pb-24 md:pb-6 space-y-4 max-w-2xl mx-auto">
          <header>
          ...
          {/* Profile Card with Background Behind Picture */}
          <div className="glass-card rounded-xl overflow-hidden">
            {/* Background Image (behind profile picture) */}
            <div className="relative">
              <div 
                className="h-32 w-full overflow-hidden"
                style={{ 
                  background: backgroundPicture ? 'none' : 'linear-gradient(135deg, var(--color-primary), var(--color-secondary))'
                }}
              >
                {backgroundPicture && (
              ...
              <button
                ...
                className="absolute top-2 right-2 p-2 bg-black/40 hover:bg-black/60 rounded-lg transition-all backdrop-blur-sm"
              ...
              <div className="absolute -bottom-10 left-4">
                <div className="relative group">
                  <div 
                    className="w-20 h-20 rounded-full flex items-center justify-center text-2xl font-black text-white overflow-hidden border-4 shadow-lg"
                    style={{ 
                      background: profilePicture ? 'none' : 'linear-gradient(135deg, var(--color-primary), var(--color-secondary))',
                      borderColor: 'var(--color-card, #1e293b)'
                    }}
                  >
                    {profilePicture ? (
